...
public class Graph<E extends Comparable<E>> implements GraphAPI<E> {
    ...
    public boolean isPath(E fromKey, E toKey) {
        ...
        //checks whether both city exists
        Vertex tmpFrom = first;
        Vertex source = null;
        while (tmpFrom != null && cmp.compare(fromKey, tmpFrom.data) > 0)
            tmpFrom = tmpFrom.pNextVertex;
        ...
        //clears previously processed
        Vertex walkPtr = tmpFrom;
        while (walkPtr != null) {
            walkPtr.processed = 0;
            if (walkPtr.data.compareTo(fromKey) == 0) {
                source = walkPtr; //locate start point
            }
            walkPtr = walkPtr.pNextVertex;
        }
        Stack<Vertex> stack = new Stack<>();
    ...
    public long outDegree(E key) throws GraphException {
        if (isEmpty())
            throw new GraphException("Non-existent vertex 2- outDegree().");
        Vertex tmp = first;
        while (tmp != null && cmp.compare(key, tmp.data) != 0)
            tmp = tmp.pNextVertex;
        if (tmp == null)
